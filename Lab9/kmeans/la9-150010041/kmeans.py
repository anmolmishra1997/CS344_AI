...
def initialization_kmeansplusplus(data, distance, k):
	...
	random_point = random.randint(0, len(data)-1)	#Random Integer for initial point
	data_point = deepcopy(data[random_point])
	means = [data_point]		#means initialized with the first point

	for cluster in range(1, k):		#loop for the next k-1 points
		dist_list = []				#for each point, stores the squared distance to the closest means point
		
		for record in data:
			mindist = float('inf')

			for cluster in means:
				dist = distance(record, cluster)
				if dist < mindist:
					mindist = dist
			dist_list.append(mindist**2)		

		dist_sum = sum(dist_list)	#sum of squared distances for normalization
		dist_norm = [dist/dist_sum for dist in dist_list]	#normalizing each point's distance

		### From here on, we use the standard method to sample from a discrete probability distribution
		### We divide the interval between 0 and 1 into n partitions with lengths proportional to the
	 	### probability mass of each point, and allot the partitions to datapoints in the order provided
	 	### in the data list. Then we sample a random number in [0,1], and pick the partition in which that
	 	### random number lies. This method ensures that we'll never pick an already picked point because
	 	### their partition lengths shall always be 0 once they've been picked.

		dist_cumulative = deepcopy(dist_norm)
		for record_num in range(1, len(data)):
			dist_cumulative[record_num] += dist_cumulative[record_num - 1]

		random_number = random.random()

		for record_num in range(len(data)):
			if random_number < dist_cumulative[record_num]:
				data_point = deepcopy(data[record_num])
...
def iteration_one(data, means, distance):
	...
	for record in data:
		...
		cluster_count[mincluster] += 1

		for j in range(dimension):
			new_means[mincluster][j] += record[j]

	for clusterIndex in range(k):
		for j in range(dimension):
			if cluster_count[clusterIndex] != 0:
				new_means[clusterIndex][j] /= 1.0 * cluster_count[clusterIndex]
			else:
...
def hasconverged(old_means, new_means, epsilon=1e-1):
	...
	converged = True
	k = len(old_means)
	dimension = len(old_means[0])


	for clusterIndex in range(k):
		dist = distance_euclidean(old_means[clusterIndex], new_means[clusterIndex])
		if dist > epsilon:
			converged = False
			break
...
def iteration_many(data, means, distance, maxiter, epsilon=1e-1):
	...
	old_means = deepcopy(means)
	all_means = [old_means]
	iter_means = []

	for i in range(maxiter):
		iter_means = iteration_one(data, old_means, distance)
		all_means.append(iter_means)
		if hasconverged(old_means, iter_means):
			break
		old_means = iter_means
